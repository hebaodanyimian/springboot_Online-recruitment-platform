<template>
	<div class="main-content" :style='{"padding":"0","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","background":"#fff","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">公司名称</label>
						<el-input v-model="searchForm.gongsimingcheng" placeholder="公司名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">企业名称</label>
						<el-input v-model="searchForm.qiyemingcheng" placeholder="企业名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">职位名称</label>
					...
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}' class="select" label="职位类型" prop="zhiweileixing">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">职位类型</label>
					...
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">福利待遇</label>
						<el-input v-model="searchForm.fulidaiyu" placeholder="福利待遇" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"0","margin":"30px 0","flexWrap":"wrap","background":"transparent","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('zhaopinxinxi','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						审核
					</el-button>
					<el-button class="btn18" v-if="isAuth('zhaopinxinxi','职位分析')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						职位分析
					</el-button>
				</el-row>
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column  :resizable='true' prop="zhaopinfengmian" width="200" label="招聘封面">
						<template slot-scope="scope">
							<div v-if="scope.row.zhaopinfengmian">
								<img v-if="scope.row.zhaopinfengmian.substring(0,4)=='http'&&scope.row.zhaopinfengmian.split(',w').length>1" :src="scope.row.zhaopinfengmian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.zhaopinfengmian)">
								<img v-else-if="scope.row.zhaopinfengmian.substring(0,4)=='http'" :src="scope.row.zhaopinfengmian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.zhaopinfengmian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.zhaopinfengmian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.zhaopinfengmian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('zhaopinxinxi','应聘')" @click="yingpinxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								应聘
							</el-button>
							<el-button class="edit" v-if=" isAuth('zhaopinxinxi','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('zhaopinxinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
		...
		<el-dialog
			...
			width="800">
			<div id="zhiweileixingChart1" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var zhiweileixingChart1 = echarts.init(document.getElementById("zhiweileixingChart1"),'macarons');
					this.$http({
						url: "zhaopinxinxi/group/zhiweileixing",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].zhiweileixing);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].zhiweileixing
								})
							}
							var option = {};
							...
							// 使用刚指定的配置项和数据显示图表。
							zhiweileixingChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								zhiweileixingChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			getDataList() {
				...
				if(this.searchForm.zhiweileixing!='' && this.searchForm.zhiweileixing!=undefined){
 				...
 				if(this.searchForm.fulidaiyu!='' && this.searchForm.fulidaiyu!=undefined){
					params['fulidaiyu'] = '%' + this.searchForm.fulidaiyu + '%'
				}
