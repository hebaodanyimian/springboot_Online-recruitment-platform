<template>
	<div class="main-content" :style='{"padding":"0","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","background":"#fff","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">工号</label>
				...
				<el-row class="actions" :style='{"padding":"0","margin":"30px 0","flexWrap":"wrap","background":"transparent","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('yingpinxinxi','应聘统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						应聘统计
					</el-button>
				</el-row>
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' prop="jianli" label="简历">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('yingpinxinxi','通知')" @click="mianshitongzhiCrossAddOrUpdateHandler(scope.row,'cross','是','','[1]','已通知')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								通知
							</el-button>
							...
							<el-button class="view" v-if="isAuth('yingpinxinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
		...
		<el-dialog
			...
			width="800">
			<div id="qiyemingchengChart1" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var qiyemingchengChart1 = echarts.init(document.getElementById("qiyemingchengChart1"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							qiyemingchengChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								qiyemingchengChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussyingpinxinxi',query:{refid:id}});
			},
			//批量审核窗口
			shBatchDialog(){
