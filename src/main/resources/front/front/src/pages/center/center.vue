<template>
	<div class="center-preview">
		...
		<div class="center-info">
			...
			<div class="info-item5" v-if="userTableName=='qiuzhizhe'">
				<span class="icon iconfont "></span>
				<div class="label">手机：</div>
				<div class="text">{{sessionForm.shouji}}</div>
			</div>
			<div class="info-item6" v-if="userTableName=='qiuzhizhe'">
				<span class="icon iconfont "></span>
		...
		<el-tabs class="center-tabs" tab-position="left" @tab-click="handleClick">
			<el-tab-pane label="个人中心">
				<el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="180px">
					<el-form-item class="center-item" v-if="userTableName=='qiuzhizhe'" label="账号" prop="zhanghao">
						<el-input v-model="sessionForm.zhanghao" placeholder="账号" readonly></el-input>
					</el-form-item>
					<el-form-item class="center-item" v-if="userTableName=='qiuzhizhe'" label="姓名" prop="xingming">
					...
					<el-form-item class="center-item" v-if="userTableName=='qiuzhizhe'" label="年龄" prop="nianling">
					...
					<el-form-item class="center-item" v-if="userTableName=='qiuzhizhe'" label="手机" prop="shouji">
						<el-input v-model="sessionForm.shouji" placeholder="手机" ></el-input>
					</el-form-item>
					<el-form-item class="center-item" v-if="userTableName=='qiuzhizhe'" label="邮箱" prop="youxiang">
						<el-input v-model="sessionForm.youxiang" placeholder="邮箱" ></el-input>
					</el-form-item>
					<el-form-item class="center-item" v-if="userTableName=='qiuzhizhe'" label="简历" prop="jianli">
						<file-upload
							tip="点击上传简历"
							action="file/upload"
							:limit="1"
							:type="3"
							:multiple="true"
							:fileUrls="sessionForm.jianli?sessionForm.jianli:''"
							@change="qiuzhizhejianliHandleAvatarSuccess"
							></file-upload>
					</el-form-item>
...
<script>
	...
	export default {
		...
		created() {
			...
			if ('qiuzhizhe' == this.userTableName) {
				this.$set(this.sessionForm, 'jianli', null);
			}
			if ('qiuzhizhe' == this.userTableName) {
			...
			if ('qiuzhizhe' == this.userTableName&&this.rules['zhanghao']){
				this.rules['zhanghao'].push({ required: true, message: '请输入账号', trigger: 'blur' })
			}else if('qiuzhizhe' == this.userTableName&&!this.rules['zhanghao']) {
			...
			if ('qiuzhizhe' == this.userTableName&&this.rules['xingming']){
				this.rules['xingming'].push({ required: true, message: '请输入姓名', trigger: 'blur' })
			}else if('qiuzhizhe' == this.userTableName&&!this.rules['xingming']) {
		...
		methods: {
			...
			onSubmit(formName) {
				...
				if(`qiuzhizhe` == this.userTableName && this.sessionForm.jianli!=null){
					this.sessionForm.jianli = this.sessionForm.jianli.replace(new RegExp(this.$config.baseUrl,"g"),"");
				}
			...
			qiuzhizhejianliHandleAvatarSuccess(fileUrls) {
				this.sessionForm.jianli = fileUrls;
			},
