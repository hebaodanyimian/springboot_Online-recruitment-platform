	...
	<div class="main-containers">
		<div class="body-containers">
			<div class="top-container">
				<!-- info -->
				<div class="top_title">
					<span @click="goMenu('/index/home')">基于Web的在线招聘平台设计与实现</span>
				</div>
				<!-- notice -->
				<el-button class="notice" @click="dialogVisible = true" type="success">
					<span class="icon iconfont icon-gonggao8"></span>
					系统公告
				</el-button>
				<el-dialog title="系统公告" :append-to-body="true" :visible.sync="dialogVisible" width="50%">
					<div class="ql-snow ql-editor" v-html="noticeDetail.content"></div>
				</el-dialog>
	...
	methods: {
		...
		async getSession() {
			await this.$http.get(`${localStorage.getItem('UserTableName')}/session`, {emulateJSON: true}).then(async res => {
				if (res.data.code == 0) {
					if(localStorage.getItem('UserTableName')== 'qiuzhizhe') {
						localStorage.setItem('username', res.data.data.zhanghao);
					}
					if(localStorage.getItem('UserTableName')== 'qiyeyonghu') {
						localStorage.setItem('username', res.data.data.gonghao);
					}
	...
	.main-containers {
		.body-containers {
			...
			.menu-preview {
				...
				top: 70px;
				left: 100px;
				background: linear-gradient(30deg, rgba(93,95,221,1) 0%, rgba(106,19,200,1) 100%);
