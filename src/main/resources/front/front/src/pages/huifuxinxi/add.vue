<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="面试时间" prop="mianshishijian">
				<el-date-picker
			...
			<el-form-item class="add-item"  label="回复" prop="huifu">
				<el-select v-model="ruleForm.huifu" placeholder="请选择回复" :disabled=" false  ||ro.huifu" >
					<el-option
						v-for="(item,index) in huifuOptions"
						:key="index"
						:label="item"
						:value="item">
					</el-option>
					...
					if(valid) {
						if(this.type=='cross'){
							...
							if(crossrefid && crossuserid) {
								...
								await this.$http.get('huifuxinxi/list', {
									params: params
								}).then(res => {
						...
						await this.$http.post(`huifuxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
